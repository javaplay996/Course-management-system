<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="学时数" prop="xueshishu">
          <el-input v-model="ruleForm.xueshishu" 
              placeholder="学时数" clearable  :readonly="ro.xueshishu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="学时数" prop="xueshishu">
              <el-input v-model="ruleForm.xueshishu" 
                placeholder="学时数" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="任课教师" prop="renkejiaoshi">
          <el-input v-model="ruleForm.renkejiaoshi" 
              placeholder="任课教师" clearable  :readonly="ro.renkejiaoshi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="任课教师" prop="renkejiaoshi">
              <el-input v-model="ruleForm.renkejiaoshi" 
                placeholder="任课教师" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.kechengxuefen = true;
          ...
          if(o=='xueshishu'){
            this.ruleForm.xueshishu = obj[o];
	    this.ro.xueshishu = true;
            continue;
          }
          if(o=='renkejiaoshi'){
    ...
    info(id) {
      this.$http({
        url: `xueshengkechengxinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `xueshengkechengxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xueshengkechengxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
