<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="分数等级" prop="fenshudengji">
          <el-select v-model="ruleForm.fenshudengji" placeholder="请选择分数等级">
            <el-option
                v-for="(item,index) in fenshudengjiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="分数等级" prop="fenshudengji">
	      <el-input v-model="ruleForm.fenshudengji"
                placeholder="分数等级" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="课程掌握程度" prop="kechengzhangwochengdu">
          <el-select v-model="ruleForm.kechengzhangwochengdu" placeholder="请选择课程掌握程度">
            <el-option
                v-for="(item,index) in kechengzhangwochengduOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="课程掌握程度" prop="kechengzhangwochengdu">
	      <el-input v-model="ruleForm.kechengzhangwochengdu"
                placeholder="课程掌握程度" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  data() {
	...
	fudaoyuanxingming : false,
      ...
      ruleForm: {
        ...
        fenshu: '',
        fenshudengji: '',
        kechengzhangwochengdu: '',
        fudaoyuanzhanghao: '',
        fudaoyuanxingming: '',
      },
          fenshudengjiOptions: [],
          kechengzhangwochengduOptions: [],
      rules: {
          suozaiyuanxi: [
          ],
          suozainianji: [
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.fenshu = true;
          ...
          if(o=='fenshudengji'){
            this.ruleForm.fenshudengji = obj[o];
	    this.ro.fenshudengji = true;
            continue;
          }
          if(o=='kechengzhangwochengdu'){
            this.ruleForm.kechengzhangwochengdu = obj[o];
	    this.ro.kechengzhangwochengdu = true;
            continue;
          }
      ...
      this.$http({
		...
		if(json.jiaoshixingming!=''&&json.jiaoshixingming){
          ...
          this.$message.error(data.msg);
            ...
            this.fenshudengjiOptions = "优秀,良好,及格,不及格".split(',')
            this.kechengzhangwochengduOptions = "优秀,良好,及格,差".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `kaoshichengjixinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `kaoshichengjixinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.kaoshichengjixinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
